...
const RoomItem = memo((props) => {
  ...
  const sliderRef = useRef()
    ...
    function controlCilckHandle(isRight = true, event) {
      isRight ? sliderRef.current.next() : sliderRef.current.prev();
      let newIndex = isRight ? selectIndex + 1 : selectIndex - 1;
      if (newIndex < 0) newIndex = itemData.picture_urls.length;
    ...
    const slideElement = (
      <div className="slider">
        <div className="control">
          <div className="btn left" onClick={e => controlCilckHandle(false, e)}>
            <IconArrowLeft width="30" height = "30"/>
          </div>
          <div className="btn right" onClick={e => controlCilckHandle(true, e)}>
          ...
          <Indicator selectIndex={selectIndex}>
            {
              itemData?.picture_urls?.map((item, index) => {
                return <span  className="dot-item" key={index}>
